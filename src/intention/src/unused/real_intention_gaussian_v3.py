...
class RealIntentionGaussian:
    ...
    def get_mean_and_covariance(self):
        p = self.eef_pose_state.get_target_frame_pose()
    ...
    def get_best_box(self):
        ...
        rv = multivariate_normal(mean=mean_2d, cov=cov_2d)
        t_max_pdf = rv.pdf(mean_2d)
    ...
    def update(self):
        ...
        rospy.loginfo(f"Best Box: {best_box.id}, Possibility: {possibility}")

        self.best_box_pub.publish(best_box)
